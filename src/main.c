...
#ifdef SELF_GRAVITY
  SelfGravityInit(); // Initialize the fft data for self-gravity
#endif
...
#ifdef DUST
  if (MAX_DUST_SPECIES > 0 && NDUST > MAX_DUST_SPECIES) {
    printf("Chosen dust algorithm does not support enough species:\n") ;
    printf("\tSupported:    %i\n", MAX_DUST_SPECIES);
    printf("\tRequested:    %i\n", MAX_DUST_SPECIES);
    prs_exit(1422);
  }
...
#ifdef SELF_GRAVITY
  // Compute the potential so that we can output it.
