...
void WriteOutputsAndDisplay(int type) {
 ...
 /* If type=ALL, all fields are dumped (This is the old fashion
  ...
  for (iDust = 0; iDust < NDUST; iDust++) {
    __WriteField(Dust_Density[iDust], TimeStep) ;
#ifdef X
    __WriteField(Dust_Vx[iDust], TimeStep) ;
#endif
#ifdef Y
    __WriteField(Dust_Vy[iDust], TimeStep) ;
#endif
#ifdef Z
    __WriteField(Dust_Vz[iDust], TimeStep) ;
#endif
  }
#endif

#ifdef SELF_GRAVITY
  __WriteField(Pot, TimeStep) ;
#endif
