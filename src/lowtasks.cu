...
extern "C" int Host2Dev2D (Field2D *F) {
 ...
 if (F->kind == XY)
    return (int)cudaMemcpy2D (F->field_gpu, (F->pitch)*sizeof(real), F->field_cpu, (Nx+2*NGHX)*sizeof(real), \
			      (Nx+2*NGHX)*sizeof(real), Ny+2*NGHY, cudaMemcpyHostToDevice);
...
extern "C" int Dev2Host2D (Field2D *F) {
  ...
  if (F->kind == XY)
     return (int)cudaMemcpy2D (F->field_cpu, (Nx+2*NGHX)*sizeof(real), F->field_gpu, (F->pitch)*sizeof(real), \
			       (Nx+2*NGHX)*sizeof(real), Ny+2*NGHY, cudaMemcpyDeviceToHost);
  return 1; //To silence a warning
}
...
extern "C" int Host2Dev2DInt (FieldInt2D *F) {
 ...
 if (F->kind == ZX)
    return (int)cudaMemcpy2D (F->field_gpu, Pitch_Int_gpu*sizeof(int), F->field_cpu, (Nx+2*NGHX)*sizeof(int), \
			      (Nx+2*NGHX)*sizeof(int),Nz+2*NGHZ,cudaMemcpyHostToDevice);

 if (F->kind == XY)
    return (int)cudaMemcpy2D (F->field_gpu, Pitch_Int_gpu*sizeof(int), F->field_cpu, (Nx+2*NGHX)*sizeof(int), \
			      (Nx+2*NGHX)*sizeof(int),Ny+2*NGHY,cudaMemcpyHostToDevice);
...
extern "C" int Dev2Host2DInt (FieldInt2D *F) {
  ...
  if (F->kind == ZX)
    return (int)cudaMemcpy2D (F->field_cpu, (Nx+2*NGHX)*sizeof(int), F->field_gpu, Pitch_Int_gpu*sizeof(int), \
			      (Nx+2*NGHX)*sizeof(int),Nz+2*NGHZ,cudaMemcpyDeviceToHost);

  if (F->kind == XY)
    return (int)cudaMemcpy2D (F->field_cpu, (Nx+2*NGHX)*sizeof(int), F->field_gpu, Pitch_Int_gpu*sizeof(int), \
			       (Nx+2*NGHX)*sizeof(int),Ny+2*NGHY,cudaMemcpyDeviceToHost);
}
