//MPI global variables

int CPU_Rank;
int CPU_Number;
boolean CPU_Master = YES;

//Global variables


boolean VxIsResidual = NO;
boolean LogGrid = NO;
boolean GuidingCenter = NO;
boolean Corotating = NO;
boolean Restart = NO;
boolean Restart_Full = NO;
boolean Stockholm = NO;
boolean Merge = YES;
boolean Merge_All = NO;
boolean MonitorIntegral;
boolean TimeInfo = NO;
boolean EverythingOnCPU = NO;
boolean ForwardOneStep = NO;
boolean Vtk2dat = NO;
boolean Dat2vtk = NO;
boolean PostRestart = NO;
boolean OnlyInit = NO;
boolean EarlyOutputRename = NO;
boolean RedefineOptions = NO;
boolean DeviceFileSpecified = NO;
boolean StretchOldOutput = NO;
boolean ThereArePlanets = NO;
boolean ThereIsACentralBinary = NO;
real    PhysicalTimeInitial;
real    PhysicalTime = 0;
real    XAxisRotationAngle = 0.0;
char    NewOutputdir[1024];
char    DefaultOut[1024];
char    DeviceFile[1024];
int     TimeStep = 0;
int     FullArrayComms = 0;
int     ContourComms = 0;
int     DeviceManualSelection = -1;
int     ArrayNb = 0;
int     StretchNumber = 0;
int     BinaryStar1 = 0;
int     BinaryStar2 = 0;

real InnerBorder;
real OuterBorder;

TimeProcess t_speedup_cpu;
TimeProcess t_speedup_gpu;
int t_speedup_count;
real time_speedup_cpu;
real time_speedup_gpu;

PlanetarySystem *Sys;
Point DiskOnPrimaryAcceleration;
Point IndirectTerm;

real step_time;

real localforce[12];
real globalforce[12];

//FIELDS VARIABLES

Grid Gridd;

Field *Vx;
Field *Vy;
Field *Vz;

Field *Vx_temp;
Field *Vy_temp;
Field *Vz_temp;

Field *Slope;

Field *Mpx;
Field *Mpy;
Field *Mpz;
Field *Mmx;
Field *Mmy;
Field *Mmz;

Field *Pot;

Field *DivRho;
Field *DensStar;
Field *Qs;

Field *Density;
Field *Energy;
Field *Pressure;

Field *QL;
Field *QR;
Field *LapPPA;

// Below: fields specific to FARGO algorithms
Field2D *VxMed;
Field2D *Vxhy;
Field2D *Vxhyr;
Field2D *Vxhz;
Field2D *Vxhzr;
Field2D *Reduction2D;
Field2D *Eta_profile_xi;
Field2D *Eta_profile_xizi;
Field2D *Eta_profile_zi;

FieldInt2D *Nxhy;
FieldInt2D *Nxhz;
FieldInt2D *Nshift;

//MHD FIELDS
//#ifdef MHD
Field *Bx;
Field *By;
Field *Bz;

Field *B1_star;
Field *B2_star;

Field *V1_star;
Field *V2_star;

Field *Slope_b1;
Field *Slope_v1;
Field *Slope_b2;
Field *Slope_v2;

Field *Emfx; 
Field *Emfy;
Field *Emfz;

Field *Divergence;
//#endif

// Dust Fields
Field **Dust_Density;
Field **Dust_Vx;
Field **Dust_Vy;
Field **Dust_Vz;
real     *Drag_Coeff;
DragData *Dust_DragData;
// Boundary location for the dust
FieldInt2D **Dust_Zmin;
FieldInt2D **Dust_Zmax;


// Stockholm BC Fields
Field2D *Density0;
Field2D *Vx0;
Field2D *Vy0;
Field2D *Vz0;
Field2D *Energy0;




//Communications variables

int Ncpu_x; // Numbers of cpus in x-axis;
int Ncpu_y; // Idem in y-axis;

Buffer Bfd;   //|
Buffer Bfu;   //|
Buffer Bfl;   //|
Buffer Bfr;   //|----> interface buffers for density field; (borders)
Buffer Bfcdl; //|                view comm.c
Buffer Bfcdr; //|
Buffer Bfcul; //|
Buffer Bfcur; //|

//Useful numbers

//CPU GLOBAL LIGHT ARRAYS
real Dx;
real *Xmin;
real *Ymin;
real *Zmin;
real *Xmed;
real *Ymed;
real *Zmed;
real *InvDiffXmed;
real *InvDiffYmed;
real *InvDiffZmed;
real *Sxj;
real *Sxk;
real *Syj;
real *Syk;
real *Szj;
real *Szk;
real *InvVj;
real shift_buffer[MAX1D];

//GPU GLOBAL LIGHT ARRAYS
real *Dx_d;
real *Xmin_d;
real *Ymin_d;
real *Zmin_d;
real *Sxj_d;
real *Sxk_d;
real *Syj_d;
real *Syk_d;
real *Szj_d;
real *Szk_d;
real *InvVj_d;
real shift_buffer_d[MAX1D];

//Grid variables

int Nx;
int Ny;
int Nz;
int J;
int K;
int Y0;
int Z0;
int Stride_cpu;
int Stride_gpu;
int Pitch_cpu;
int Pitch_gpu;
int Pitch_Int_gpu;
int Pitch2D;
int Stride;
int ix;
int iy;
int ycells;
int zcells;
int y0cell;
int z0cell;

//For checknan
Field *ListOfGrids = NULL;

//Boundary variables

int Bounl;
int Bounr;
int Bounu;
int Bound;

//psys variables.
real Xplanet;
real Yplanet;
real Zplanet;
real VXplanet;
real VYplanet;
real VZplanet;
real MplanetVirtual;

MPI_Status fargostat;

real OMEGAFRAME0;

int Fscan;

long VtkPosition = 0; 

//Pointers to functions
//WARNING!!! FUNCTIONS' ARGUMENTS MUST NOT CONTAIN BLANK SPACES
void (*ComputePressureFieldIso)();
void (*ComputePressureFieldAd)();
void (*ComputePressureFieldPoly)();
void (*SubStep1_x)(real);
void (*SubStep1_y)(real);
void (*SubStep1_z)(real);
void (*SubStep2_a)(real);
void (*SubStep2_b)(real);
void (*SubStep3)(real);
void (*SubStep_drag_x)(real,int,real);
void (*SubStep_drag_y)(real,int,real);
void (*SubStep_drag_z)(real,int,real);
void (*Diffuse_dust_x)(real,Field*,Field*);
void (*Diffuse_dust_y)(real,Field*,Field*);
void (*Diffuse_dust_z)(real,Field*,Field*);
void (*DivideByRho)(Field*,Field*);
void (*VanLeerX_a)(Field*);
void (*VanLeerX_b)(real,Field*,Field*,Field*);
void (*VanLeerY_a)(Field*);
void (*VanLeerY_b)(real,Field*,Field*,Field*);
void (*VanLeerZ_a)(Field*);
void (*VanLeerZ_b)(real,Field*,Field*,Field*);
void (*momenta_x)(Field*,Field*);
void (*momenta_y)(Field*,Field*);
void (*momenta_z)(Field*,Field*);
void (*reduction_SUM)(Field*,int,int,int,int); 
void (*reduction_MIN)(Field*,int,int,int,int); 
void (*UpdateX)(real,Field*,Field*,Field*);
void (*UpdateY)(real,Field*,Field*,Field*);
void (*UpdateZ)(real,Field*,Field*,Field*);
void (*UpdateDensityX)(real,Field*,Field*);
void (*UpdateDensityY)(real,Field*);
void (*UpdateDensityZ)(real,Field*);
void (*NewVelocity_x)(Field*,Field*);
void (*NewVelocity_y)(Field*,Field*);
void (*NewVelocity_z)(Field*,Field*);
void (*AdvectSHIFT)(Field*,FieldInt2D*);
void (*ComputeResidual)(real,Field*,Field*);
void (*ChangeFrame)(int,Field*,Field2D*);
void (*Potential)();
void (*CorrectVtheta)(real,Field*);
void (*cfl)();
void (*cfl_dust)(int);
void (*_ComputeForce)(real,real,real,real,real);
void (*copy_velocities)(int,Field*,Field*,Field*);
void (*VanLeerX_PPA_a)(Field*);
void (*VanLeerX_PPA_b)(Field*);
void (*VanLeerX_PPA_steep)(Field*);
void (*VanLeerX_PPA_c)(Field*);
void (*VanLeerX_PPA_d)(real,Field*,Field*,Field*);
void (*VanLeerX_PPA_d_2d)(real,Field*,Field*,Field2D*);
void (*mon_dens)();
void (*mon_momx)();
void (*mon_momy)();
void (*mon_momz)();
void (*mon_torq)();
void (*mon_reynolds)();
void (*mon_maxwell)();
void (*mon_bxflux)();
void (*comm)();
//MHD..........................................
void (*ComputeSlopes)(int,int,int,Field*,Field*);
void (*_ComputeStar)(real,int,int,int,int,int,int,int,int,int,Field*,Field*,Field*,Field*,Field*,Field*,Field*,Field*,Field*,Field*);
void (*_ComputeEmf)(real,int,int,int,int,int,int,Field*,Field*,Field*,Field*,Field*,Field*,Field*,Field*,Field*);
void (*_UpdateMagneticField)(real,int,int,int,int,int,int,int,int,int,Field*,Field*,Field*);
void (*_LorentzForce)(real,int,int,int,int,int,int,int,int,int,int,int,Field*,Field*,Field*,Field*,Field*);
void (*_Resist)(int,int,int,int,int,int,int,int,int,Field*,Field*,Field*,Field2D*);
void (*EMF_Upstream_Integrate)(real);

void (*StockholmBoundary)(real);
void (*visctensor_cart)();
void (*addviscosity_cart)(real);
void (*visctensor_cyl)();
void (*addviscosity_cyl)(real);
void (*visctensor_sph)();
void (*addviscosity_sph)(real);
void (*boundary_ymin)();
void (*boundary_zmin)();
void (*boundary_ymax)();
void (*boundary_zmax)();
void (*boundary_ymin_dust)(int,real);
void (*boundary_zmin_dust)(int,real);
void (*boundary_ymax_dust)(int,real);
void (*boundary_zmax_dust)(int,real);
void (*Fill_GhostsX)();
void (*CheckMuteY)();
void (*CheckMuteZ)();

void (*SetupHook1)();

void (*__WriteField)();
void (*__Restart)(Field*,int);

// Self gravity
void (*SelfGravityInit)(void);
void (*SelfGravityPotential)(void);
