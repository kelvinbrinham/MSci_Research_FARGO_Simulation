...
void ChangeArch() {
  ...
  boundary_ymin_dust  = boundary_dust_ymin_cpu;
  boundary_ymax_dust  = boundary_dust_ymax_cpu;
  boundary_zmin_dust  = boundary_dust_zmin_cpu;
  boundary_zmax_dust  = boundary_dust_zmax_cpu;
  ...
  //DUST-----------------------------------------------
  SubStep_drag_x = SubStep_drag_x_cpu ;
  SubStep_drag_y = SubStep_drag_y_cpu ;
  SubStep_drag_z = SubStep_drag_z_cpu ;
  cfl_dust = cfl_dust_cpu ;
  Diffuse_dust_x = Diffuse_dust_x_cpu ;
  Diffuse_dust_y = Diffuse_dust_y_cpu ;
  Diffuse_dust_z = Diffuse_dust_z_cpu ;

  //SELF GRAVITY---------------------------------------
#ifdef SELF_GRAVITY
  SelfGravityInit = SelfGravityInit_cpu ;
  SelfGravityPotential = SelfGravityPotential_cpu ;
#endif
  ...
  while (fgets(s, MAXLINELENGTH-1, func_arch) != NULL) {
	...
	if(strval[0] == 'g'){
	  ...
	  boundary_ymin_dust  = boundary_dust_ymin_gpu;
	  boundary_ymax_dust  = boundary_dust_ymax_gpu;
	  boundary_zmin_dust  = boundary_dust_zmin_gpu;
	  boundary_zmax_dust  = boundary_dust_zmax_gpu;
	...
	if(strval[0] == 'g'){
      ...
      //DUST-----------------------------------------------
      if (strcmp(name, "dust") == 0){ 
	if (strval[0] == 'g'){
	  SubStep_drag_x = SubStep_drag_x_gpu ;
	  SubStep_drag_y = SubStep_drag_y_gpu ;
	  SubStep_drag_z = SubStep_drag_z_gpu ;
	  cfl_dust = cfl_dust_gpu ;
	  printf("Dust Substep1 runs on the GPU\n");
	  
	  Diffuse_dust_x = Diffuse_dust_x_gpu ;
	  Diffuse_dust_y = Diffuse_dust_y_gpu ;
	  Diffuse_dust_z = Diffuse_dust_z_gpu ;
	  printf("Dust diffusion runs on the GPU\n");
	}
      }
      //SELF GRAVITY-----------------------------------------------
#ifdef SELF_GRAVITY
      if (strcmp(name, "selfgravity") == 0){ 
	if (strval[0] == 'g'){
	  SelfGravityInit = SelfGravityInit_gpu ;
	  SelfGravityPotential = SelfGravityPotential_gpu ;
	  printf("Self-gravity runs on the GPU\n");
	}
      }
#endif
