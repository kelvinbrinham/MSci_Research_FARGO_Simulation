...
void CreateFields() {
  ...
  Nshift = CreateFieldInt2D ("Nshift", YZ);
  Nxhy   = CreateFieldInt2D ("Nxhy", YZ);
  Nxhz   = CreateFieldInt2D ("Nxhz", YZ);
#elif defined(POTENTIAL)
  /* Needed as a temporary */
  Vx_temp     = CreateField("Vx_temp", VXTEMP, 1,0,0);
  Mpx         = CreateField("Moment_Plus_X" , 0, 1,0,0);
  Mmx         = CreateField("Moment_Minus_X", 0, 1,0,0);
#endif
...
#ifdef DUST
  if (NDUST > 0) {
    Dust_Density = (Field **) malloc(NDUST * sizeof(Field*)) ;
    Dust_Vx      = (Field **) malloc(NDUST * sizeof(Field*)) ;
    Dust_Vy      = (Field **) malloc(NDUST * sizeof(Field*)) ;
    Dust_Vz      = (Field **) malloc(NDUST * sizeof(Field*)) ;
    
    Drag_Coeff    = (real *)     malloc(NDUST * sizeof(real)) ;
    Dust_DragData = (DragData*)  malloc(NDUST * sizeof(DragData)) ;
    
    Dust_Zmin = (FieldInt2D**) malloc(NDUST * sizeof(FieldInt2D*));
    Dust_Zmax = (FieldInt2D**) malloc(NDUST * sizeof(FieldInt2D*));

    int n ; 
    char dust_name[128] ;
    for (n=0; n < NDUST; ++n) {
      sprintf(dust_name, "dustdens%i_",n) ;
      Dust_Density[n] = CreateField(dust_name, DUST_DENS, 0,0,0) ;
#ifdef X
      sprintf(dust_name, "dustvx%i_",n) ;
      Dust_Vx[n] = CreateField(dust_name, DUST_VX, 1,0,0) ;
#endif
#ifdef Y
      sprintf(dust_name, "dustvy%i_",n) ;
      Dust_Vy[n] = CreateField(dust_name, DUST_VY, 0,1,0) ;
#endif
#ifdef Z
      sprintf(dust_name, "dustvz%i_",n) ;
      Dust_Vz[n] = CreateField(dust_name, DUST_VZ, 0,0,2) ;
#endif
      sprintf(dust_name, "dustzmin%i_",n) ;
      Dust_Zmin[n] = CreateFieldInt2D(dust_name, XY);

      sprintf(dust_name, "dustzmax%i_",n) ;
      Dust_Zmax[n] = CreateFieldInt2D(dust_name, XY);
      
      // Initialize Zmin/Zmax to boundary locations:
      int i, j;
      for (j = 0; j < Ny + 2*NGHY; j++)
	for (i = 0; i < Nx; i++) {
	  Dust_Zmin[n]->field_cpu[l2D_XY] = NGHZ ;
	  Dust_Zmax[n]->field_cpu[l2D_XY] = Nz + NGHZ ;	  
	}
    }
  }
#endif
...
int RestartSimulation(int n) {
  ...
  for (iDust = 0; iDust < NDUST; iDust++) {
    __Restart(Dust_Density[iDust], n) ;
#ifdef X
    __Restart(Dust_Vx[iDust], n) ;
#endif
#ifdef Y
    __Restart(Dust_Vy[iDust], n) ;
#endif
#ifdef Z
    __Restart(Dust_Vz[iDust], n) ;
#endif
  }
#endif
...
// Save the drag coefficients to file
void SaveDragCoefficients() {
  int i ;
  FILE *fd ;
  char filename[256] ;
  sprintf(filename,"%sdrag_coefficients.dat",OUTPUTDIR);
...
// Load the drag coefficients from file - will be useful after a restart
void LoadDragCoefficients()  {
  ...
  if (CPU_Master){ 
    if ((fd = fopen(filename, "r")) == NULL) {
      ...
      for (i = 0; i < NDUST; i++)
	{
	  nRead = fscanf(fd, "%lf", Drag_Coeff+i) ;
	  if (nRead != 1)
	    {
	      printf("Failed to read Drag coefficient\n") ;
	      fflush(stdout) ;
	      exit(1) ;
	    }
	}     
...
#ifdef FLOAT
  MPI_Bcast(Drag_Coeff, NDUST, MPI_FLOAT, 0, MPI_COMM_WORLD) ;
#else
  MPI_Bcast(Drag_Coeff, NDUST, MPI_DOUBLE, 0, MPI_COMM_WORLD) ;
#endif
}
