...
void AlgoGas () {
  ...
  if (Resistivity_Profiles_Filled == NO) {
...
#ifdef DUST
    if (NDUST > 0) {
      printf("Dust not support in MHD as initial velocity is lost") ;
      prs_exit(1422) ;	
    }
#endif    
    // THIS COPIES Vx_temp INTO Vx
    FARGO_SAFE(copy_velocities(VTEMP2V, Vx, Vy, Vz));
    ...
    VxIsResidual = NO;
#endif
    FARGO_SAFE(copy_velocities(V2VTEMP, Vx, Vy, Vz));
    ...
    // DO THE DUST FORCES ------------------------------------------------------
#ifdef DUST
    InitSpecificTime(&t_drag, "Drag Forces");

    for (iDust=0; iDust < NDUST; iDust++) {

#if ((defined(SHEARINGSHEET2D) || defined(SHEARINGBOX3D)) && !defined(SHEARINGBC))
      FARGO_SAFE(NonReflectingBC(Dust_Vy[iDust]));
#endif


      // Y - Z - X ordering avoids copies, allowing updates in place
#ifdef Y
      FARGO_SAFE(SubStep_drag_y(dt, iDust, Drag_Coeff[iDust])) ;
#endif
#ifdef Z
      FARGO_SAFE(SubStep_drag_z(dt, iDust, Drag_Coeff[iDust])) ;
#endif
#ifdef X
      FARGO_SAFE(SubStep_drag_x(dt, iDust, Drag_Coeff[iDust])) ;
#endif
...
#endif
#endif
    /// DO THE DUST TRANSPORT STEP
    transport(dt, Density, 
    ...
    for(iDust=0; iDust < NDUST; iDust++)
      {
	FARGO_SAFE(copy_velocities(V2VTEMP, Dust_Vx[iDust], Dust_Vy[iDust], Dust_Vz[iDust])) ;

	transport(dt, Dust_Density[iDust], 
		  Vx_temp, Vy_temp, Vz_temp,
		  Dust_Vx[iDust], Dust_Vy[iDust], Dust_Vz[iDust], 0);
      }
#endif
