...
/* Physical data for drag forces. 
 *
 * Note: the velocities must be evaluated at the end of the step.
 */ 
struct drag_data {
  real Kdrag; 
  real density;
...
/* Data for fftw domain decomposition*/
struct fft_grids {
  int local_Nx, local_i_start;
  int Ny, stride_y;
  int local_size;
};


/* Data required for fftw computation*/
struct sg_domain {
  int rank ;
  int start, end ;
  int offset ;
...
struct sg_comm {
  struct sg_domain * sends; // Where our mesh data goes
  struct sg_domain * recvs; // Where our fft data comes from
  int Nsend, Nrecv ;
} ;
