...
class Setup():
    def __init__(self,setup,boundaries,centering,dust=False):
        self.dust = dust
        self.fields = pformat_read(setup,"\w+")
        self.library = Library(boundaries,centering)
        fields = []
        for field in self.fields:
            if not self.dust and     'dust_' in field['name']: continue
            if     self.dust and not 'dust_' in field['name']: continue
            f = Field(field['name'],self.library)
            f.field = field['name'].capitalize()
            if f.field.startswith('Dust_'):
                f.field = 'Dust_' + field['name'][5:].capitalize()
...
class Boundary():
    ...
    def write_output(self):
        if not self.dust:
            ...
            output = open(self.side+"_dust_bound.c","w")
    ...
    def process_ifields(self):
        ...
        for field in self.setup.fields:
            ...
            if not self.dust:
                ...
                ifields_lines += "INPUT(" + field.field + "[iDustSpecies]);\n"
        n = self.stones[string]
        self.template.template[n] = \
    ...
    def process_ofields(self):
        ...
        for field in self.setup.fields:
            ...
            if not self.dust:
                ...
                ofields_lines += "OUTPUT(" + field.field + "[iDustSpecies]);\n"
        n = self.stones[string]
        self.template.template[n] = \
...
def write_mute(side, template, dust=False):
    ...
    if not dust:
        ...
        output1 = open(side+"min_dust_bound.c","w")
    ...
    if not dust:
        ...
        output2 = open(side+"max_dust_bound.c","w")
...
if __name__ == '__main__':
    ...
    # Dust boundaries:
    template = Template(STDDIR+'boundary_template_dust.c')

    try:
        setup = Setup(SETUP, BOUNDARIES, CENTERING, dust=True)
    except:
        write_mute("y",copy.deepcopy(template), dust=True)
        write_mute("z",copy.deepcopy(template), dust=True)
        print "================================="
        print "Warning: Some file is missing..." 
        "\nCheck the files: \n" + SETUP + "\n" \
            + BOUNDARIES + "\n" + CENTERING + \
    ...
    try:
        ...
        write_mute("y", copy.deepcopy(template), dust=True)
        print "Skipping dust boundaries in Y. Not defined."
    ...
    try:
        ...
        write_mute("z", copy.deepcopy(template), dust=True)
        print "Skipping dust boundaries in Z. Not defined."
